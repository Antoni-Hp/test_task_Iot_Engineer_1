...
def upload_data(data, headers):
    ...
    for keys in data:
        for unique_headers in headers:   # listuje pojedyńcze unikatowe nagłówki
            if unique_headers not in keys:
                keys[unique_headers] = 'Brak danych'
        add_data_to_table(headers, keys)
        if keys[issue_headers[0]] != '' and keys[issue_headers[0]] != 'Brak danych':    #uzupełnia wartości dla brakujących kluczy
            i_heating.append(keys[issue_headers[0]].strip())
            issues[0] = len(i_heating)
        elif keys[issue_headers[1]] != '' and keys[issue_headers[1]] != 'Brak danych':
            i_motion.append(keys[issue_headers[1]].strip())
        elif keys[issue_headers[2]] != '' and keys[issue_headers[2]] != 'Brak danych':
            i_temp_sensor.append(keys[issue_headers[2]].strip())
        elif keys[issue_headers[3]] != '' and keys[issue_headers[3]] != 'Brak danych':
            i_door.append(keys[issue_headers[3]].strip())
        elif keys[issue_headers[4]] != '' and keys[issue_headers[4]] != 'Brak danych':
            i_window.append(keys[issue_headers[4]].strip())
        elif keys[issue_headers[5]] != '' and keys[issue_headers[5]] != 'Brak danych':
            i_heat_switch.append(keys[issue_headers[5]].strip())
        elif keys[issue_headers[6]] != '' and keys[issue_headers[6]] != 'Brak danych':
            i_kit_switch.append(keys[issue_headers[6]].strip())
        elif keys[issue_headers[7]] != '' and keys[issue_headers[7]] != 'Brak danych':
    ...
    for i in [i_heating, i_motion, i_temp_sensor, i_door, i_window, i_heat_switch, i_kit_switch, i_wifi]:
        issues.append(len(i))
    tab = Table(displayName="Tabela1", ref=f"A1:{get_column_letter(data_sheet.max_column)}{data_sheet.max_row}")    #tworzy tabele o zakresie od A1 do ostatniej komórki z danymi
    ...
    chart_issues(issues, last_cell)
    for issues in [i_heating, i_motion, i_temp_sensor, i_door, i_window, i_heat_switch, i_kit_switch, i_wifi]:
        last_cell += 18
        last_cell = chart_issue(issues, last_cell, issue_cat)
        issue_cat += 1
    try:
        wb.save(excel_file_name)   #zapis pliku excel
    except PermissionError:
        print(f"Plik {excel_file_name} jest otwarty i nie można nadpisać zmian. Zamknij plik excel")


def add_data_to_table(headers, data):
...
def chart_issues(issues, last_cell):
...
def chart_issue(issue_heating, last_cell, issue_cat):
